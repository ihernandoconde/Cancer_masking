...
button.addEventListener('click', function () {
    ...
    if (upload_button.files.length === 1) {
        ...
        eel.processing_image(fileContent)(function (result) { //callback function
            // Set the processed image as the src of the image element
            document.getElementById('breast_one').src = result;
            document.getElementById("birads_explanation").innerHTML = paragraph;
        })//redirects page
    }
    if (upload_button.files.length === 2) {
        fileContent.push(upload_button.files[0]);
        ...
        eel.processing_image(fileContent)(function (result) { //callback function
            // Set the processed image as the src of the image element
            document.getElementById('breast_one').src = result;
            document.getElementById("breast_two").src = result;
            document.getElementById("birads_explanation").innerHTML = result;
        })
    }
    if (upload_button.files.length < 1) {
